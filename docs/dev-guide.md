...
To test on a mobile device on the same network (e.g. via ZeroTier VPN), expose the dev server on all interfaces:

```bash
npm run dev -- --host
```

Vite will print all bound addresses including the ZeroTier IP. Open that on your phone.

> **Note:** The mobile/desktop split (`isMobile`) is determined once in `onMount` via `window.innerWidth`. Resizing a desktop browser window after load will not correctly re-evaluate this — the Three.js canvas path or mobile grid path is locked in at page load time. This only affects desktop browser resize testing; real mobile devices (which start at narrow width) work correctly.

## Resume PDFs

PDF files in `public/resume*.pdf` are generated by `src/print-resume.js` using Puppeteer. Run manually with the dev server running:

```bash
npm run resume
```

A GitHub Actions workflow (`.github/workflows/generate-resume.yml`) also runs this automatically on every push to `main` that touches `src/content/resume*.md`, and commits the updated PDFs back to the repo.

To add a new resume variant:
1. Create `src/content/resume-<key>.md`
2. Add `"<key>": outPath` to `VARIANTS` in `src/print-resume.js`
3. Add the same canonical entry to `canonicalVariants` in both `src/pages/resume.astro` and `src/pages/resume-print.astro`

> Due to this, run `git pull` before any pushing, if not sure whether the action has pushed new pdf files or not.
...
| Variable | Location | Example value | Effect |
|---|---|---|---|
...
| What | Where | Example value | Effect |
|---|---|---|---|
